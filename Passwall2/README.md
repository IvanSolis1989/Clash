# Passwall2 使用教程（对齐 Clash Party v6.0.0）

> 目录简介：这里提供 Passwall2 精简分流版的 fused shunt rule fallback、UCI 批量脚本和 OpenWrt 导入说明。
>
> 版本：**v6.0.0-pw2.2**（Build 2026-07-09；基线：Clash Party v6.0.0；变更历史见 `Passwall2/CHANGELOG.md`）。
...
本目录只提供 fallback：从最终 fused 规则集生成 68 条 Passwall2 原生 shunt rule。每条 rule 只引用一个远程 `.srs`：
...
| 文件 | 用途 |
| --- | --- |
| `Passwall2(xray+sing-box)-apply.sh` | SSH 到路由器后执行，一次性创建 68 条 fused shunt rule |
| `Passwall2(xray+sing-box).conf` | 纯文本参考，展示 68 条 rule 的顺序、目标和 `.srs` URL |
...
| --- | --- | --- |
| fused 规则顺序 | 68 条 shunt rule | 130 条 Mihomo 主规则 |
