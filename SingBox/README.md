# SingBox 使用教程（对齐 Clash Party v6.0.8 Full 语义）

> 目录简介：这里维护 sing-box Full JSON 生成脚本、生成产物和 Hiddify/HomeProxy 等 sing-box 客户端导入教程。
>
> 配置文件：`SingBox/SingBox(sing-box)-full.json`（v6.0.8-sing.1；urltest interval 默认 5m；126 个 Mihomo bucket provider 合并为 65 个唯一非空 `.srs`，生成资产使用发布版本缓存键）
