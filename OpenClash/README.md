...
|------|------|---------------------------|
| `OpenClash(mihomo).conf` | OpenClash **UI 配置快照**：把推荐的 UCI 选项（核心 = Smart、DNS = fake-ip、Sniffer、GeoX 自动更新、LightGBM 自动更新等 30+ 项）一次性灌进 OpenClash，省掉手工逐项勾选 | LuCI → OpenClash → **覆写设置** 页面里的 **配置文件上传**入口（一次性导入，不需要落到固定路径） |
...
这一步会把核心类型 = Smart、DNS = fake-ip、Sniffer、GeoX / LightGBM 自动更新等推荐选项一次性写入 OpenClash，避免你手动去勾 30+ 项。
...
不强制，但**强烈推荐**。`.conf` 里的 UCI 选项（DNS / Sniffer / GeoX / LightGBM 自动更新等）是覆写脚本默认假设的运行环境；手动配置等价选项需要在 LuCI 里勾 30+ 项，且容易漏。已经熟悉 OpenClash 并手动调好的用户可以跳过。
