...
const V2RAYN_VERSION = 'v6.0.5-v2n.1';
const PASSWALL_VERSION = 'v6.0.5-pw.1';
const PASSWALL2_VERSION = 'v6.0.5-pw2.1';
...
function passwallRuleUrl(segment) {
  return `${FUSED_SRS_BASE_URL}/${segment.id}.srs`;
}
...
function renderPasswallScript({ appName, version, title, configName, nodeComment, segments, baselineVersion }) {
  ...
  const addCalls = segments.map((segment, index) => (
    ...
    + `add_fused_shunt_rule '${segment.id} | ${segment.policy}' '${passwallRuleUrl(segment)}' '${hasIpPayload(segment) ? '1' : '0'}'`
  )).join('\n\n');
...
function renderPasswallConf({ title, version, readmePath, segments, baselineVersion }) {
  ...
  segments.forEach((segment, index) => {
    const url = passwallRuleUrl(segment);
...
function renderPasswallList(segment) {
  return [
    ...
    `rule-set:remote:${passwallRuleUrl(segment)}`,
    '',
  ].join('\n');
}
