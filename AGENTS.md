...
显式不支持的客户端（架构原因，写独立产物也只能给 10% 功能）：
- **Passwall / Passwall2 / SSR Plus+**：路由模型只有 proxy/direct/block 三出站，**无法**表达 28 业务组 × 9 区域组；这些用户应迁移到 OpenClash（`OpenClash/README.md` 顶部有迁移步骤）
