# Loon 使用教程（对齐 Clash Party v6.0.3）

> 目录简介：这里维护 Loon iOS 配置和使用教程，按 Loon 原生语法对齐 Clash Party 分流策略。
>
> 配置文件：`Loon/Loon.conf`
> 版本：**v6.0.3-Loon.1**（Build 2026-07-12，详见 `Loon/CHANGELOG.md`；跟随 Clash Party v6.0.3 基线；68 个源语义段对应 64 个非空远程规则）
> 目标：**Loon iOS（App Store 付费正版）**
> 架构：22 区域 url-test 组（11 全部 + 11 家宽，[Remote Filter] NameRegex）+ 33 业务策略组 + 64 个非空融合 [Remote Rule]
...
1. Loon → **首页** → 应显示 `Loon Smart v6.0.3-Loon.1`，协议已启用。
